...
import { ExhibitionHostVenueFields } from "@/components/exhibitions/ExhibitionHostVenueFields";
...
export default function EditExhibitionPage() {
  ...
  const [venueNameKo, setVenueNameKo] = useState("");
  ...
  useEffect(() => {
    ...
    Promise.all([getExhibitionById(id), getMyProfile()]).then(
      ([{ data: exData, error: exErr }, { data: profileData }]) => {
        ...
        setVenueNameKo(data.venue_name_ko ?? "");
        setVenueNameEn(data.venue_name_en ?? "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const legacyHost =
      ...
      null;
    const legacyVenue =
      pickLegacyForSave(venueNameKo || null, venueNameEn || null) ??
      venueName.trim() ??
      null;
    const { error: err } = await updateExhibition(
      id,
      {
        ...
        venue_name: legacyVenue || venueName.trim() || null,
        venue_name_ko: venueNameKo.trim() || null,
        venue_name_en: venueNameEn.trim() || null,
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            */}
            ...
            <ExhibitionHostVenueFields
              radioName="host_profile_edit"
              forProfileId={effectiveProfileId}
              hostProfileMode={hostProfileMode}
              onHostProfileModeChange={setHostProfileMode}
              hostNameKo={hostNameKo}
              hostNameEn={hostNameEn}
              onHostNamesChange={({ legacy, ko, en }) => {
                setHostName(legacy);
                setHostNameKo(ko);
                setHostNameEn(en);
              }}
              hostSelected={hostSelected}
              onHostSelectedChange={setHostSelected}
              venueNameKo={venueNameKo}
              venueNameEn={venueNameEn}
              onVenueNamesChange={({ legacy, ko, en }) => {
                setVenueName(legacy);
                setVenueNameKo(ko);
                setVenueNameEn(en);
              }}
