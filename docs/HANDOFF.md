...
### 미완성("설정 중인 프로필") 제외
- `listPublicProfiles`(작가 탭 전용 소스)에서 display_name 없음 + placeholder
  username(`user_<hex>`) 프로필을 목록에서 제외. 정확히 "설정 중인 프로필"로
  렌더되던 카드만 제거(링크 불가·빈 카드). 커서는 DB 윈도 기준이라 무한스크롤
  정상 진행. (공개 작가 중 11개 해당)
