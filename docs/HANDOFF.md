...
업로드 **이미지 보정**에서 작품 영역(녹색 쿼드) 단계인데도 분홍 배너
「이 사진은 사용이 어려워요」가 계속 자리를 차지하던 문제.

원인은 `block` 배너가 항상 렌더되고, 「그래도 계속」은 Save/Enhance
차단만 푸는 `override`만 켰기 때문. 이제는 같은 세션·같은 파일에서
배너를 닫고(`dismissed` + `override`), 밝기/강도 재실행에도 다시
안 뜬다. 새 사진을 고르면 게이트는 다시 나올 수 있다. 「재촬영」은
그대로. 떠나기 확인 대화상자는 넣지 않음.

**Verified:** `npx tsx tests/quality-gate-banner-dismiss.test.ts`,
`npx tsc --noEmit`.
...
업로드 **이미지 보정** 중에 둘러보기·메시지 등 사이드바 탭이 눌리지
않던 문제. 업로드를 끝내지 못하게 잠근 것이 아니라, 루트에 떠 있는
업로드 투어 오버레이가 클릭을 가로채고 있었다.

- 스포트라이트 SVG / 타깃 없는 전체 딤: `pointer-events-none` (시각만).
  클릭은 Skip/Next/Done 팝오버만 받음.
- `role="dialog"` + `aria-modal="true"` 제거 → `role="region"` (코치마크).
- 스텝 타깃이 사라지면 전체 화면 클릭 차단 대신 그 스텝(또는 투어)을 스킵.
- 투어 페이지를 떠나면 (`/upload` → `/feed` 등) 활성 투어를 스킵해서
  오버레이가 앱을 따라다니지 않음.
- `ImageStandardizeEditor`에는 전체 뷰포트 클릭 트랩 없음. 떠나기 확인
  대화상자는 넣지 않음.

**Verified:** `npx tsx tests/tour-nav-not-trapped.test.ts`, `npx tsc --noEmit`.

---

## 2026-08-22 (61) — 원작 맞춤 슬라이더가 갤러리 여백을 건드리지 않음

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

밝기·대비·채도 미세조정이 결과 이미지 전체에 들어가 `#f3f3f3` 벽까지
같이 변하던 문제. 레시피 `bezel`로 작품 사각형만 찾아 그 안에만 적용.
벽과 여백 안 그림자는 그대로.

**Verified:** `userFineTune` 유닛 테스트(벽 픽셀 불변, 작품 픽셀 리프트).

---

## 2026-08-22 (60) — 업로드 새로고침 흰 화면

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

업로드(데스크톱) 새로고침 시 사이드바까지 사라진 흰 화면이 멈추던 문제.
Safari에서 탭이 여러 개면 supabase-js `getSession()`이 Navigator Lock에
걸려 AuthGate가 영원히 `ready=false`로 남고, 셸 라우트는 헤더도 숨긴다.
잠금을 건너뛰고 4초면 화면을 열어 두며, 토큰 갱신 때 `router.refresh()`로
게이트가 다시 비지 않게 했다. 업로드 레이아웃은 대기 중에도 사이드바를
그린다.

**Verified:** 코드 경로. 실기기 Safari 다중 탭 새로고침은 배포 후 확인.

---

## 2026-08-22 (59) — 보정 되돌리기 계층 정리와 슬라이더 미리보기 깜빡임

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

원본으로 되돌리기는 Quick 조정의 display_adjust만 리셋해서 AI 화면에서 죽은 버튼이었음. 처음으로=전체 다시, 그 아래 작품 보정 되돌리기=같은 크롭에서 조명/슬라이더만 초기화. 슬라이더 미리보기 크로스페이드. 고급설정(진단) UI 제거, 선택값은 accepted 이벤트 메타로만 수집.

**Verified:** `npx tsc --noEmit`. SQL/env 없음.

---

## 2026-08-22 (58) — 보정 결과에 밝기·대비·채도 미세조정 슬라이더

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

보정 결과에서 강도(약하게/추천/강하게)와 촬영 방식을 고른 뒤, **원작에 더 가깝게 맞추기**로 밝기·대비·채도를 제한 슬라이더(±30%)로 맞출 수 있음. 예전 “입력 유형”은 조명이 아니라 스캐너/스튜디오/자동 **촬영 방식**이므로 라벨을 바꿈. 슬라이더는 크롭·원근 파이프라인을 다시 돌리지 않고, 이미 나온 결과 위에 후처리한다.

**Verified:** `npx tsc --noEmit`, recipe `userFineTune` roundtrip, 후처리 픽셀 수식 유닛 테스트.

---

## 2026-08-22 (57) — 보정 결과에서 확정 크롭이 강도 변경 때 원본으로 되돌아가던 문제

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

강도(약하게/추천/강하게)나 입력 유형을 바꾸면, 이미 확정한 작품 크롭이
빠지고 원본 스튜디오 사진(바닥·벽·주변)이 "보정 결과"에 다시 보이던
회귀. 확정 모서리는 `perspectiveCornersRef`로 유지하고, 결과 화면은
원본 `previewUrl` fallback을 쓰지 않으며, 새 결과가 올 때까지 이전
크롭 미리보기를 유지한다. 레시피 effect는 분석 완료를 기다리지 않고,
미리보기 identity에 묶이지 않는다.

**Verified:** 코드 경로. 실사진 회귀는 업로드에서 확인.

---

## 2026-08-22 (56) — 보정 강도·입력 유형이 미리보기에 반영되지 않던 버그

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

강도(약하게/추천/강하게)와 입력 유형(자동/스튜디오/스캐너)을 바꿔도
미리보기가 그대로였다. 슬라이더 debounce가 매 렌더 새 객체로 타이머를
리셋해, 버튼 변경이 파이프라인까지 도달하지 못했다. 이제 옵션 키가
바뀌면 바로 다시 돌리고, 진행 중인 결과는 최신 것만 화면에 올린다.

---

## 2026-08-22 (55) — 갤러리 여백 균등 + 보정 플로우 UX

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

- 표준 여백: 그림자를 아래쪽에 캔버스로 더 붙이던 패딩을 제거. 작품이
  상하좌우 같은 간격의 `#f3f3f3` 벽 정중앙에 오도록 맞춤. 그림자는 그
  여백 안에만 그림.
- 최종 버튼 카피: "이 조명으로 사용" → "이 이미지 사용". 단계 제목은
  "보정 결과".
- 보정 강도(약하게/추천/강하게)가 밝기·대비·노출에 실제로  commited.
- "이 이미지 사용" 후 에디터를 접지 않음. 결과 미리보기가 그대로 남고
  "이어서 보정"으로 이 결과부터 다시 다듬음. 승인 blob URL을 에디터가
  revoke 하지 않아 썸네일이 깨지지 않음.

---

## 2026-08-22 (54) — 보정 미리보기: 변환 후 원본 벽·바닥 제거

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

작품 크롭·정면 복원은 파일에 이미 반영돼 있었다. 조명 단계 미리보기가
원본 폰 사진 비율의 before/after 위에 결과를 `object-contain` 해서,
저장본에 없는 벽·바닥이 위아래로 비쳤다. 변환 후에는 결과 이미지
비율로 결과만 보여 준다 (`#f3f3f3`).

---

## 2026-08-22 (53) — AI 보정: 작품 영역 확정 후 키스톤 복원

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

스튜디오 폰 사진에서 바닥·옆 작품까지 캔버스로 들어가던 문제를 고친다.
AI 경로는 조명으로 바로 가지 않고 **작품 네 모서리를 확인**한 뒤에만
크롭·원근 복원·여백·조명을 돌린다.

- 자동 감지는 피커 시드만. AABB(외접 사각형) fallback 제거 — 키스톤이
  있으면 벽/바닥이 함께 잘리던 경로.
- 경계를 확신하지 못하면 유저가 모서리를 직접 찍기 전까지 진행 불가.
- 확정된 사다리꼴을 코너 90도 직사각형으로 펴고, 원작 비율 유지 후
  `#f3f3f3` 갤러리 여백 → 조명.

**Verified:** 에디터 흐름(AI → 작품 영역 → 조명) 코드 경로 점검. 실사진
회귀는 업로드에서 확인 필요.

---

## 2026-08-22 (52) — AI 보정: 메인 작품 인식 + 키스톤 복원 + 갤러리 여백

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.** (`OPENAI_API_KEY` 기존 재사용)

스튜디오에서 기울여 찍은 폰 사진에 여백만 붙던 문제를 고친다.
AI 경로가 gpt-4o 비전으로 **메인 캔버스 네 꼭짓점**을 찾고, 그 사각형으로
크롭·원근 복원 후 `#f3f3f3` 벽에 그림자 있는 표준 여백을 붙인다.
원작 가로세로비는 유지. 여러 점이 한 장에 있어도 가장 큰 완성 캔버스를
고르도록 bbox 프롬프트를 수정.

---

## 2026-08-22 (51) — 업로드 보정: 원본 vs AI 선택 흐름

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

작품 사진 첨부 직후 보정 패널이 열린다. 첫 화면은 Quick/Enhance 탭이
아니라 **원본 그대로 vs AI 작품 이미지 보정**.

- 원본: 기하·색 미변경, `#f3f3f3` 표준 여백만 넣고 바로 저장.
- AI: 자동으로 기하 복원 + 작품 영역 크롭 + 표준 여백 → 추천 조명
  (대비·명도·채도) → 선택적 "원작에 더 가깝게 맞추기".
- 원작 비율(4:3 강제 등)은 바꾸지 않는다. 여백은 짧은 변의 6%.
...
배너는 루트 레이아웃에 있어 로그인 후 대부분의 화면에 뜰 수 있다.
다만 `profile_completed_at IS NULL` 만으로 켜면, 이름·역할이 이미 있는
기존 유저까지 100%가 아닌 한 계속 노출된다.

이제 배너는 **이름 또는 대표 역할이 비어 있고**, 완성도가 70 미만일
때만 보인다. 연령대는 선택. 그 외 빈 칸은 Settings 완성도 카드에서
링크로 안내한다.

---

## 2026-08-20 (48) — 로그인·가입 와이어프레임 여백 복원

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

조밀하게 줄였던 섹션 간격을 시안의 쾌적한 리듬으로 되돌림. 제품 결정
(Google-only, 단일 이름, gender optional, Step 4 전원 등)은 불변.

- 캔버스 `min-h-screen` + `py-20/28`, 카피→폼 `mb-14`, 로고→태그라인
  `mb-8`
- 로그인 컬럼을 가입과 같은 `max-w-md`, 캡슐 기본 패딩 복구 (compact
  제거), 필드 `space-y-5`, Quick Start `mt-12`
- 가입 Step 2/3 필드 간격 `space-y-6`

---

## 2026-08-20 (47) — /login 로고·캐치프레이즈 좌정렬 복원

> **Supabase SQL 적용 필요: 없음.**
>
> **환경 변수 추가/변경: 없음.**

로그인 히어로 로고/태그라인에 넣었던 `text-center` / `justify-center` 를
제거. 페이지 컬럼은 뷰포트 중앙에 두되, 로고·캐치프레이즈·폼은 가입
Step N 과 같이 **컬럼 기준 좌정렬**. Quick Start 한 줄(Google)과
푸터 약관은 와이어프레임대로 중앙.
...
### 배경
- 직전 (9) 미니멀 패스에서 컬러를 스택바 하나로 몰아넣었더니 "어떤 초록 세그먼트가 어떤 페르소나인지" 시각 링크가 끊겼다. 헤더의 라이브 펄스 dot은 반대로 의미 없이 시선만 잡음.

### 변경 요약
- **헤더 라이브 펄스 dot 제거**. "커뮤니티" 텍스트만 남기고 우상단 얇은 `→` 는 유지.
- **레전드 앞 컬러 dot 4개 복원** (바 세그먼트 색과 동일: emerald-700/600/500/400). 이번엔 wrap 걱정 없는 `grid-cols-4` 안에 dot + 라벨을 `flex items-center justify-center gap-1` 로 단정하게 앉힘. 각 셀은 여전히 (dot+라벨) 위에 카운트 스택.
- 결과: 바-레전드 색이 시각적으로 짝지어져 "43은 아티스트, 12는 큐레이터" 를 한눈에 매칭 가능하면서, wrap 은 여전히 원천 차단.
...
### 배경
- 방금 나간 커뮤니티 펄스 스트립이 "**{합계}명 활동 중**"으로 표시됐는데, 이는 `count_personas` RPC가 다중 페르소나 회원을 **역할마다 한 번씩** 카운트하기 때문에(정의된 동작; `src/lib/supabase/personaCounts.ts:16` 주석) 합치면 unique 회원 수를 과장한다. 실측 대비: `auth.users` 71 · 실제 프로필(ops 콘솔) 59 · 페르소나 합 85.

### 변경 요약
- 스트립에서 **합산 총계 텍스트를 제거**하고, 대신 우측에 `network.persona.live` ("실시간") 캡션. 라이브 점은 유지.
- 스택바 아래에 **페르소나별 개별 카운트** 라인 추가 (`● 아티스트 43 · ● 큐레이터 12 · ● 갤러리스트 13 · ● 컬렉터 17`). 네트워크 페이지 legend와 동일한 데이터·색 언어를 컴팩트하게 표시, wrap 허용.
- 이렇게 하면 각 숫자는 정직하고 유저가 합쳐 읽지 않도록 시각적으로 분리됨.
- 사용 안 하게 된 i18n 키 `rail.community.active` 삭제 (EN/KO 둘 다). 주석에 rationale 명시.
- 후속: unique 회원 수 RPC(`count_unique_profiles` 또는 `count_personas` 확장) 도입 시 정직한 헤드라인 숫자를 다시 붙일 수 있음.
...
### 변경 요약
- 알림 전체 목록의 **← 피드**를 없앰. 사이드바가 있는 화면이라 피드로 보내는 것이 틀림.
- 보낸 문의의 돌아가기를 피드가 아니라 **워크스페이스**로 맞춤.
- 알림·문의·클레임·네트워크·룸에서 작품으로 갈 때 출발지를 남김. 이전 피드 기록이 남아 잘못 돌아가지 않게 함.
- 내 전시 상세에 같은 주소로 두 개 달리던 링크를 **← 전시** 하나로 정리.

### Verified
- `npx tsc --noEmit`

## 2026-08-17 (4) — 전시에서 연 작품은 전시로 돌아가기

> Supabase SQL 돌려야 할 것은 없음 · 환경 변수 변경 없음

### 변경 요약
- 작품 상세 돌아가기가 `/e/` 전시 게시물을 모르고 항상 피드로 가던 문제를 고침. 전시에서 들어오면 **전시 게시물**로 돌아감.
- 저장된 출발지가 없어도 같은 사이트 이전 페이지를 참고함. 알 수 없는 주소는 피드로 버리지 않음.
- 내 전시·작품 추가·보드에서 작품으로 갈 때도 출발지를 남김.
...
### 변경 요약
- 전시 만들기·수정에서 호스트(갤러리·장소) 위에 드롭다운을 둠. **내 계정**과 예전에 쓴 주최 이름을 고를 수 있음.
- 새 전시는 기본값이 **나**(프로필 연결). 피드 크레딧이 프로필 이름을 따라가 예전처럼 스냅샷 이름과 어긋나지 않게 함.
- 예전 이름만 고르면 텍스트로 저장. 다른 프로필에 연결된 이름이면 그 프로필을 다시 연결.
- 자유 입력과 기존 라디오(텍스트 / 나 / 검색)는 그대로.

### Verified
- `npx tsc --noEmit`

## 2026-08-17 — 모바일 메뉴가 네트워크·보드 입구

> Supabase SQL 돌려야 할 것은 없음 · 환경 변수 변경 없음

### 변경 요약
- 폰·태블릿에서 네트워크와 보드는 피드를 늘리지 않고 **햄버거 메뉴**에서 들어감. 메뉴를 열면 초대 한 줄, 보드 소식 한 줄이 미리 보임.
- 연결 요청이 있을 때만 메뉴 버튼에 작은 점이 생김. 보드 소식은 점으로 알리지 않음.
- 데스크톱 왼쪽 메뉴에도 **보드**가 추가됨. 넓은 화면의 오른쪽 칸은 그대로.

### Verified
- `npx tsc --noEmit`

## 2026-08-14 (5) — 햄버거 메뉴 바깥 탭은 닫기만

> Supabase SQL 돌려야 할 것은 없음 · 환경 변수 변경 없음

### 변경 요약
- 모바일·태블릿에서 메뉴를 연 뒤, 메뉴 밖을 누르면 뒤 작품·링크가 눌리지 않고 메뉴만 닫힘.
- 어두운 막을 페이지 전체에 올리고, 메뉴를 닫은 뒤에도 잠깐 뒤 화면이 눌리지 않게 막음. 피드 작품이 같은 터치로 열리지 않음.

### Verified
- Header 스크림을 body에 올리고, 바깥 터치를 캡처 단계에서 차단

## 2026-08-14 (4) — 모바일 내비·터치·설정 일관성

> Supabase SQL 돌려야 할 것은 없음 · 환경 변수 변경 없음

### 변경 요약
- 태블릿에서도 휴대폰처럼 **메뉴(햄버거)** 가 열림. 좁은 가로 메뉴 줄은 없앰. 큰 화면(`lg+`) 사이드바는 그대로.
- 메뉴에 **네트워크**가 생김. 워크스페이스 허브에서도 공개 범위 옆에 네트워크 링크. 타일은 5개 그대로.
- 알림 목록과 설정에서 **알림 설정** 페이지로 바로 갈 수 있음.
- 비공개 프로필의 뒤로가기는 **사람 탐색으로**로 맞춤 (주소는 `/people`).
- CSV로 작품 가져오기 화면이 한국어·영어로 맞춰지고, 버튼이 둥근 칩으로 통일됨.
- 설정에서 도시·지역·나라가 좁은 화면에서 세로로 쌓임. 사용자 이름이 있으면 **공개 프로필에서 탭·작품 관리** 칩이 보임. 영어 잔여 문구를 번역함. 빌드 스탬프는 맨 아래만 남김.
- 위임 가이드가 햄버거 안 계정 전환에도 붙음. 위임 중 배너는 좁은 화면에서 줄바꿈.
- 운영 콘솔 사용자·운영진 목록이 휴대폰에서 카드로 보임. 내부 뷰 이름은 숨김. 권한 회수 버튼은 둥근 알약.
- 작품 여러 개를 고를 때 휴대폰 하단에 전체 선택·삭제·취소 바가 붙음.
- 공개 프로필 탭이 가로로 스크롤되고, 탭 설정 버튼은 작은 화면에서 아래 줄에 놓임. 메뉴·프로필 사진·가이드 버튼은 손가락으로 누르기 쉽게 키움.
- 아이폰 노치·홈 바에 맞춰 위아래 여백을 줌. 가짜 하단 탭바는 없음.
- 작품 목록 제목이 **내 작품**(영문 Works)으로 바뀜. 주소(`/my/library`)는 그대로.

### Verified
- `npx tsc --noEmit`

## 2026-08-14 (3) — 워크스페이스 잔재 정리와 공개 프로필·설정 일관성

> Supabase SQL 돌려야 할 것은 없음 · 환경 변수 변경 없음

### 변경 요약
- `/my` 는 워크스페이스 허브로 정리. 헤더에 **공개 프로필** 칩(공개 페이지로 이동)과 가이드 버튼을 둠. 한국어 타일 제목은 초안·문의·소유·내 전시·출처.
- 전시 상세의 죽은 뒤로가기(`/my?tab=exhibitions`)를 `/my/exhibitions` 로 고침.
- 예전 스튜디오 가이드는 워크스페이스 안내로 바꿈. 탭 설정은 공개 프로필에서 한다고 안내. `/my/following`·`/my/followers` 는 네트워크 탭으로 보냄.
- 작업실(`/my/library`)에서 여러 작품을 골라 한 번에 지울 수 있음. 확인 창이 뜸.
- 설정에서 학력 입력칸을 없앰. 학력·전시·수상·레지던시는 **이력 편집**(`/my/profile/cv`)에서만 고침. 설정 저장이 이력을 덮어쓰지 않음.
- 공개 프로필 작가의 말이 한국어·영어 둘 다 있으면, 사이트 언어를 바꾸지 않고 카드 안에서 한국어 | English 로 볼 수 있음.
- 설정·알림·외부 작가·공개 범위·운영 콘솔에 사이드바 셸과 둥근 칩 버튼을 맞춤. 알림 페이지에서 설정·알림함으로 갈 수 있고, 출처(초대) 페이지에서 외부 작가 목록으로 갈 수 있음.
...
### 최소 완성도 게이트 + 탭명 "작가"→"아티스트"
- **탭명**: `feed.tab.artists` KO "작가" → **"아티스트"** (role chip 라벨과 통일).
- **완성도 게이트**: `listPublicProfiles`(아티스트 탭 전용 소스)에서 플랫폼 정식
  기준 `needs_identity_setup = false`(20260421 identity_completeness)와 정렬 —
  **비placeholder 핸들 + display_name + 최소 1개 role** 을 모두 갖춘 프로필만 노출.
  기준 미달 계정은 "설정 중인 프로필"로 렌더되고 링크도 불가하므로 전면 소개하지
  않음. 커서는 DB 윈도 기준이라 무한스크롤 정상 진행. (공개 작가 중 11개 미달 제외)
- 참고: People 페이지 페르소나 카운터는 이 미완성 계정도 슬롯으로 집계 중
  (34 아티스트). 목록과 수치를 완전히 일치시키려면 `count_personas`에도 동일
  가드를 넣어야 함(미적용, 필요 시 별도 결정).

### 카드 개선(저해상도/빈 아바타)

`ExploreArtistCard` 두 가지 문제 수정.

1. **저해상도**: 큰 4:3 타일에 96px `avatar` 변형을 써서 깨져 보이던 것을
   `medium`(1200px) 변형으로 교체 → 선명. (외부 http 아바타는 원본 그대로.)
2. **빈 회색칸**: 아바타 미등록 시 큰 회색 박스+흐린 글자 → **이름 기반 결정적
   그라데이션 모노그램**(id/username 해시로 색상 고정, 1~2자 이니셜)로 교체.
   사진 유무와 무관하게 "사람들의 벽"이 균일하게 보기 좋게 채워짐.

### Verified
- `tsc`/`build` 통과, 로그아웃 `/feed?tab=artists` 브라우저 실측(모노그램·고해상도 확인).
- 미완성 제외 필터: 공개 작가 중 11개 해당 확인, `tsc`/`build` 통과.
...
3. **실시간**: `profiles`를 `supabase_realtime` 퍼블리케이션에 추가. 신규 가입/
   ...
   폴백: 60초 폴링 + 포커스 시 재조회.
4. **UI**: `PersonaCountPanel.tsx` — 큰 히어로 카드가 아니라 **슬림 스티키 바**
   (전역 헤더 `top-14` 바로 아래 고정, 스크롤해도 유지, `z-30`). 한 줄 인라인
   카운트 + live 도트, 진입 시 0→N 카운트업, `prefers-reduced-motion` 존중,
   무의존성 rAF. 사람 추천 리스트가 상단으로 올라오도록 시각 비중 축소.
